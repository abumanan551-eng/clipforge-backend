...
# 8 پریمیم کیپشن اسٹائلز کی مکمل کنفیگریشن
def get_caption_style_config(style_name: str):
    # ASS کلرز فارمیٹ: &HAABBGGRR (Blue, Green, Red)
    styles = {
        "hormozi": {
            "primary": "&H0000FFFF",   # بولڈ پیلا
            "outline": "&H00000000",   # کالا آؤٹ لائن
            "border_style": "1",
            "outline_w": "4",
            "shadow": "2"
        },
        "mrbeast": {
            "primary": "&H000033FF",   # برائٹ ریڈ
            "outline": "&H0000FFFF",   # پیلا آؤٹ لائن
            "border_style": "1",
            "outline_w": "5",
            "shadow": "3"
        },
        "neon": {
            "primary": "&H0000FF00",   # نیون گرین
            "outline": "&H00000000",
            "border_style": "1",
            "outline_w": "4",
            "shadow": "4"
        ...
        "box": {
            "primary": "&H00FFFFFF",   # سفید ٹیکسٹ
            "outline": "&H00000000",
            "border_style": "3",       # بلیک اوپیک باکس بیک گراؤنڈ
            "outline_w": "3",
            "shadow": "0"
        },
        "pink": {
            "primary": "&H00FF00CC",   # ہاٹ پنک
            "outline": "&H00000000",
            "border_style": "1",
            "outline_w": "4",
            "shadow": "2"
...
def create_ass_file(filepath: str, text: str, start_sec: float, duration: float, template: str):
...
Style: Default,Arial Black,75,{cfg['primary']},-1,{cfg['outline']},&H80000000,-1,0,0,0,100,100,0,0,{cfg['border_style']},{cfg['outline_w']},{cfg['shadow']},2,60,60,360,1
...
"""
    words = text.split()
    if not words:
        words = ["THE", "SECRET", "TO", "SUCCESS", "REVEALED"]
...
def cut_video_clips(payload: CutRequest):
    ...
    for i, clip in enumerate(payload.clips):
        ...
        if payload.caption_style != "none":
            ...
            caption_text = clip.caption_text or clip.title or "VIRAL MOMENT"
            create_ass_file(ass_path, caption_text, start_sec, duration, payload.caption_style)
